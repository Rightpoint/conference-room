...
namespace RightpointLabs.ConferenceRoom.Web.Areas.Admin.Controllers
{
    public class SelectController : BaseController
    {
        ...
        public JsonResult Rooms()
        {
            ...
            return Json(rooms.Select(_ => new { id = _.Id, text = string.Format("{0} - {1} - {2}", buildings.TryGetValue(_.BuildingId), floors.TryGetValue(_.FloorId), _.RoomAddress) }).OrderBy(_ => _.text), JsonRequestBehavior.AllowGet);
        }
    }
}